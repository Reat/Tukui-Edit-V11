...
local function Shared(self, unit)
	...
	self:Tag(masterlooter, '[Tukui:pmlooter]')
...
oUF:Factory(function(self)
	...
	local party = self:SpawnHeader("oUF_TukuiHealParty", nil, "party", 
														"showParty", true, 
														"showPlayer", Unitframes.showplayerinparty, 
														"xOffset", TukuiDB.Scale(3),
														"point", "LEFT")
														
	if Unitframes.healermode then
		party:SetPoint("BOTTOM", TukuiDataBottom, "TOP", 0, TukuiDB.Scale(165))
	else
		party:SetPoint("BOTTOMLEFT", TukuiChatLeftTabs, "TOPLEFT", 0, TukuiDB.Scale(3))
	end
end)
